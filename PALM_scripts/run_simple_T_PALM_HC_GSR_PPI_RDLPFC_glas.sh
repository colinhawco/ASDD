...
#SBATCH --output=/projects/ttan/ASDD/Data1/PALM/logs/PALM_HC_GSR_PPI_RDLPFC_glas%j.txt
#SBATCH --error=/projects/ttan/ASDD/Data1/PALM/logs/PALM_HC_GSR_PPI_RDLPFC_glas%jerr.txt
#SBATCH --array=1-3
...
sublistids="${DIR}/subid_GSR/HC_sublist_GSR.txt"
filename="$(find $DIR/filelists/HC/PPI_RDLPFC_glas/filelist* -type f | head -n $SLURM_ARRAY_TASK_ID | tail -n 1)"
truncname="$(echo $(basename `echo "$filename"`) | sed 's/filelist/HC_GSR_PPI_RDLPFC_glas_con/;s/.txt//')"
