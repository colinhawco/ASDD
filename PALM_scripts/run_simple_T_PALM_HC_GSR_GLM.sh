...
#SBATCH --output=/projects/ttan/ASDD/Data/PALM/logs/PALM_HC_noGSR_GLM%j.txt
#SBATCH --error=/projects/ttan/ASDD/Data/PALM/logs/PALM_HC_noGSR_GLM%jerr.txt
#SBATCH --array=1-3
...
filename="$(find $DIR/filelists/HC/GLM/filelist* -type f | head -n $SLURM_ARRAY_TASK_ID | tail -n 1)"
truncname="$(echo $(basename `echo "$filename"`) | sed 's/filelist/HC_GSR_GLM_con/;s/.txt//')"
