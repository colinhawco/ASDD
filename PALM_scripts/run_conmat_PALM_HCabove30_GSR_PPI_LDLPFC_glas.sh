...
#SBATCH --output=/projects/ttan/ASDD/Data1/PALM/logs/PALM_HCabove30_GSR_LDLPFC_glas%j.txt
#SBATCH --error=/projects/ttan/ASDD/Data1/PALM/logs/PALM_HCabove30_GSR_LDLPFC_glas%jerr.txt
#SBATCH --array=1-3
...
sublistids="${DIR}/subid_GSR/HCabove30_sublist_GSR.txt"
filename="$(find $DIR/filelists/HCabove30/PPI_LDLPFC_glas/filelist* -type f | head -n $SLURM_ARRAY_TASK_ID | tail -n 1)"
truncname="$(echo $(basename `echo "$filename"`) | sed 's/filelist/HCabove30_GSR_PPI_LDLPFC_glas_con/;s/.txt//')"
outdir="${DIR}/Results/$truncname"
desmat="$DIR/con_design/HCabove30_design_TT.csv"
conmat="$DIR/con_design/HCabove30con_TT.csv"
